...
const completeInterfaceAll = (interfaceObj, options) => __awaiter(void 0, void 0, void 0, function* () {
    let newOptions = Object.assign({ path: "./", output: "[name].swagger2.d.ts", name: "name" }, (options || {}));
    let str = "";
    for (let key in interfaceObj) {
        str += exports.completeInterface(key, interfaceObj[key]) + "\n";
    }
    try {
        if (!fs.existsSync(newOptions.path)) {
            yield new Promise((resolve, reject) => {
                fs.mkdir(newOptions.path, { recursive: true }, (err) => {
                    if (!err) {
                        resolve(1);
                    }
                    else {
        ...
        console.log("文件地址", `${newOptions.path}/${newOptions.output.replace(/\[\S*\]/g, newOptions.name)}`);
        fs.writeFile(`${newOptions.path}/${newOptions.output.replace(/\[\S*\]/g, newOptions.name)}`, str, () => { });
    }
    catch (error) { }
});
...
//# sourceMappingURL=handleStr.js.map